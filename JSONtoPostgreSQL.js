    ...
    // to https://sequelize.org/docs/v6/core-concepts/model-instances/#creating-an-instance
     ...
     for (const entry of jsonData.entries) {
      await Listing.create({
        code: entry.code,
        name: entry.name,
        coorLatitude: entry.coordinates?.latitude || null,
        coorLongitude: entry.coordinates?.longitude || null,
        address: entry.address,
      });
    }
    console.log('Data from listings.json has been saved to the database.');
  });
} catch (error) {
