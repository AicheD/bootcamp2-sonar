   ...
   */
    async function findLibraryWest() {
       ...
       try {
        ...
        console.log(JSON.stringify(libraryWest, null, 2));
      } catch (error) {
    ...
    */
      async function removeCable() {
         //ADD CODE HERE
         try {
          const cableBuilding = await Listing.findOne({
            where: {
              code: 'CABL',
            },
          });
          ...
          if (cableBuilding) {
            await cableBuilding.destroy();
            console.log('Removed Cable BLDG');
          } else {
            console.log('Cable BLDG not found');
          }
        } catch (error) {
    ...
    */
    async function updatePhelpsLab() {
       ...
       try {
        ...
        if (phelpsLab) {
          phelpsLab.address = '1953 Museum Rd, Gainesville, FL 32603';
          await phelpsLab.save();
          console.log('Updating PhelpsLab:', JSON.stringify(phelpsLab, null, 2));
        } else {
